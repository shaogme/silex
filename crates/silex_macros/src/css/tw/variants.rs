...
pub fn tw_variants_impl(ts: TokenStream) -> Result<TokenStream> {
    ...
    let pos_arg_tuple = pos_args.iter();
    let pos_arg_opt_tuple = pos_args.iter();
    ...
    Ok(quote! {
        {
            ...
            impl TwVariantsHelper {
                ...
                pub fn get(&self, #(#pos_args: #pos_arg_types),*) -> ::std::string::String {
                    #(#pos_arg_eval)*
                    let key = ( #(#pos_arg_tuple),* );
                    let base_cls = match key {
                        #(#match_arms,)*
                        _ => #default_class_name,
                    };
