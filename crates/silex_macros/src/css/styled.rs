...
pub fn styled_impl(input: TokenStream) -> Result<TokenStream> {
    ...
    let attrs_binding = attrs_binding.unwrap_or_else(|| {
        ...
        all_fn_args.push(syn::parse_quote! {
            #[attrs] #ident: #__view::attributes::AttributeGroup<#scope>
        });
        ident
    });
    ...
    let node_view = quote! {
        {
            use #__view::attributes::{AttributeBuilder as _, GlobalAttributes as _};
            #node_init
                .class(#class_name)
                #style_prop_binding
                .apply(#__view::attributes::AttrOp::CombinedStyles(#__view::attributes::CombinedStyles::new(
                    ::std::vec![],
                    ::std::vec![ #(#style_bindings),* ],
                    ::std::vec![],
                )))
...
fn process_dynamic_entries(input: DynamicEntryExpansion<'_>) -> Result<()> {
    ...
    for (i, (prop, expr)) in entries.iter().enumerate() {
        ...
        style_bindings.push(quote! {
            #__view::attributes::ReactiveBindingPlan::style_property(
...
fn get_tag_return_type(
    ...
    if let Some(spec) = tag_spec {
        ...
        quote! { #__view::elements::TypedElement<#scope, #__silex::html::#marker> }
    } else {
        quote! {
            impl #__view::attributes::AttributeBuilder<#scope>
                + #__view::mount::View<#scope>
                + #scope
                #where_clause
        }
    }
...
pub fn global_impl(input: TokenStream) -> Result<TokenStream> {
    ...
    Ok(quote! {
        ...
        #vis fn #c_name #fn_generics(
            #params
        ) -> #__silex::core::SilexResult<impl #__view::mount::View<#scope> + #scope>
            #where_clause
        {
            #assertions
            #static_value_decls
...
fn generate_static_global(input: StaticGlobalExpansion<'_>) -> Result<TokenStream> {
    ...
    Ok(quote! {
        ...
        #vis fn #name #fn_generics(
            #params
        ) -> impl #__view::mount::View<#scope> + #scope
            #where_clause
        {
            #assertions
            #static_value_decls
            let #static_values_ident: ::std::vec::Vec<::std::string::String> =
                #static_value_tokens;
            #static_inits
            use #__view::mount::View;
            #__view::mount::View::into_any(())
        }
    })
}
