...
impl<T, C> HttpClientBuilder<T, C> {
    ...
    pub(crate) fn apply_interceptors(&self, spec: &mut RequestSpec) {
        for hook in &self.before_send {
            hook(spec);
        }
    }

    pub(crate) fn notify_error(&self, spec: &RequestSpec, err: &NetError) {
    ...
    pub(crate) fn notify_response(&self, spec: &RequestSpec, response: &HttpResponse) {
