...
pub fn generate_macro_tables(
    ...
    let mut static_entries: Vec<(String, Vec<(&'static str, String)>)> = Vec::with_capacity(candidate_set.len());
    ...
    table_code.push_str("// 自动生成的 Tailwind 静态规则表与 PHF 映射表（供 silex_macros 使用）\n");
    ...
    table_code.push_str("use phf::phf_map;\n");
    ...
    table_code.push_str("pub static STATIC_RULES: phf::Map<&'static str, &'static [(&'static str, StaticVal)]> = phf_map! {\n");
    ...
    table_code.push_str(r#"pub fn resolve_static_rule(
    modifiers: &[Modifier],
    utility_token: &str,
    span: Span,
...
"#);
    ...
    table_unimplement_code.push_str("// 自动生成的 Tailwind 未匹配/未实现静态类名表（供 silex_macros 使用）\n");
...
fn parse_val_to_static_val(val: &str) -> String {
    ...
    if val.contains('(') || val.contains(' ') || val.contains('/') || val.contains(',') {
        ...
        {
            return format!("StaticVal::Kw(\"{}\")", val.replace('\\', "\\\\").replace('"', "\\\""));
        }
        return format!("StaticVal::Literal(\"{}\")", val.replace('\\', "\\\\").replace('"', "\\\""));
    }
    format!("StaticVal::Kw(\"{}\")", val.replace('\\', "\\\\").replace('"', "\\\""))
}
