...
impl Runtime {
    ...
    #[inline]
    fn for_each_subscriber(&self, id: NodeId, mut f: impl FnMut(NodeId)) {
    ...
    pub(crate) fn propagate(&self, start_node: NodeId, queue: &mut VecDeque<NodeId>) {
        queue.clear();

        // 直接订阅者一律 Dirty。
        self.for_each_subscriber(start_node, |sub_id| {
            if self.storage.get_state(sub_id) != NodeState::Dirty {
                self.storage.set_state(sub_id, NodeState::Dirty);
                self.schedule_or_walk(sub_id, queue);
            }
        });

        while let Some(current_id) = queue.pop_front() {
            self.for_each_subscriber(current_id, |sub_id| {
                // 只把 Clean 提升到 Check —— 绝不把已经 Dirty 的节点降级。
                if self.storage.get_state(sub_id) == NodeState::Clean {
                    self.storage.set_state(sub_id, NodeState::Check);
                    self.schedule_or_walk(sub_id, queue);
                }
            });
        }
    ...
    #[inline]
    fn schedule_or_walk(&self, id: NodeId, queue: &mut VecDeque<NodeId>) {
        if self.storage.node(id).is_some_and(ReactiveNode::is_effect) {
...
mod tests {
    ...
    impl Graph {
        ...
        fn edge(&self, from: NodeId, to: NodeId) {
            let version = self.rt.storage.node(from).map_or(0, |n| n.version.get());
            self.rt
                .storage
                .node(from)
                .expect("from 必须存在")
        ...
        fn state(&self, id: NodeId) -> NodeState {
            self.rt.storage.get_state(id)
        }

        fn set_state(&self, id: NodeId, state: NodeState) {
            self.rt.storage.set_state(id, state);
        }
        ...
        fn propagate_from(&self, start: NodeId) -> Vec<NodeId> {
            ...
            self.rt.propagate(start, &mut queue);
    ...
    fn propagate_ignores_nodes_that_no_longer_exist() {
        ...
        assert!(
            g.rt.storage.node(dead).is_none(),
    ...
    fn a_check_node_whose_dependencies_changed_is_recomputed() {
        ...
        let (a, b) = (g.signal(), g.memo());
        g.edge(a, b);
        g.rt.storage.node(a).expect("a 活着").bump_version();
        g.set_state(b, NodeState::Check);
    ...
    fn the_cycle_report_names_every_node_on_the_cycle() {
        ...
        let report = g.rt.describe_cycle(&frames, c);
