...
//!
//! 从前整个 crate 只对外交出一个 `NodeId`，而 signal / memo / derived / effect /
...
impl<K: NodeKind> Handle<K> {
    ...
    pub const DANGLING: Self = Self::from_raw(RawNodeId::DANGLING);

    #[inline(always)]
    pub(crate) const fn from_raw(raw: RawNodeId) -> Self {
        Self {
            raw,
            _kind: PhantomData,
        }
    ...
    #[inline(always)]
    pub const fn from_raw_unchecked(raw: RawNodeId) -> Self {
        Self::from_raw(raw)
    }
}
...
/// 任何句柄 —— 用于对种类无所谓的操作（[`dispose`](crate::scope::dispose)、
/// 调试标签、原始指针逃生出口）。
pub trait AnyHandle: Copy + sealed::Sealed {
    /// 擦除种类。
    fn to_raw(self) -> RawNodeId;
}
...
impl sealed::Sealed for RawNodeId {}
impl AnyHandle for RawNodeId {
    #[inline(always)]
    fn to_raw(self) -> RawNodeId {
        self
    }
...
///
/// [`RawNodeId`] 也实现了它 —— 那是**类型擦除的逃生出口**，把种类检查退回到
...
mod tests {
    ...
    fn a_typed_handle_costs_nothing() {
        assert_eq!(
            ...
            std::mem::size_of::<RawNodeId>()
        );
        assert_eq!(
            std::mem::align_of::<SignalId>(),
            std::mem::align_of::<RawNodeId>()
        );
