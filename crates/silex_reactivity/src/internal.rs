...
impl NodeId {
    ...
    /// Returns `true` if this `NodeId` is the dangling sentinel.
    #[inline]
    pub(crate) fn is_dangling(self) -> bool {
        self == Self::DANGLING
    }

    /// Returns `true` if this `NodeId` represents a valid non-dangling node key.
    #[inline]
    pub(crate) fn is_valid(self) -> bool {
        self != Self::DANGLING
    }

    /// Converts an `Option<NodeId>` to a `NodeId`, replacing `None` with
    /// `NodeId::DANGLING`.
    #[inline]
