//! 阶段三内部架构与 B4-B6 安全容器的行为固化。
//!
//! 这一轮把节点拆成独立的 meta / links / value / computation 表，图算法改为
//! 原地遍历 SoA，并把 arena / sparse map 收紧为安全的独占访问。公开 API 一个字
//! 都没动，因此这里断言的是**它买回来的东西**：
//!
...
/// | 阶段三旧实现 | 写：debug 断言 / release `Reentrant`；读路径依赖内部借用 |
/// | B4-B6（现在） | **一律** [`ReactiveError::Reentrant`]，安全容器不允许别名写入 |
///
...
///
/// B4-B6 把访问入口收成一次**独占**借用之后，读与写不再有区别：
/// `clone` 里的运行时调用会直接返回 `Reentrant`，不再需要用户维护 arena 引用
/// 的残留契约。
///
