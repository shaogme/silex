...
fn a_panicking_memo_recomputes_on_the_next_read() {
    ...
    let m = memo::create::<i32, _>(move |_| {
        r.set(r.get() + 1);
        let first = signal::try_get::<i32>(a).unwrap();
        if e.get() {
            panic!("boom");
        }
        // panic 的那一轮跑不到这里，b 因此没有被登记成依赖。
        first + signal::try_get::<i32>(b).unwrap()
    });
    ...
    signal::update::<i32>(b, |v| *v = 10);
    assert_eq!(signal::try_get::<i32>(m), Ok(12));
}
...
fn the_runtime_still_works_after_a_cycle_panic() {
    ...
    let _ = signal::try_get::<i32>(first);
    ...
    // 一条与环完全无关的链，必须照常工作。
    let fresh = signal::create(10i32);
    let doubled = memo::create::<i32, _>(move |_| signal::try_get::<i32>(fresh).unwrap() * 2);
    let seen = Rc::new(Cell::new(0));
    let sc = seen.clone();
    effect::create(move || sc.set(signal::try_get::<i32>(doubled).unwrap()));

    assert_eq!(seen.get(), 20);
    signal::update::<i32>(fresh, |v| *v = 21);
...
fn a_panicking_effect_still_reacts_to_its_registered_dependencies() {
    let registered = signal::create(0i32);
    let never_read = signal::create(0i32);
    let explode = Rc::new(Cell::new(false));
    let runs = Rc::new(Cell::new(0));

    let (e, r) = (explode.clone(), runs.clone());
    effect::create(move || {
        let _ = signal::try_get::<i32>(registered);
        r.set(r.get() + 1);
        if e.get() {
            panic!("boom");
        }
        let _ = signal::try_get::<i32>(never_read);
    });

    assert_eq!(runs.get(), 1);

    explode.set(true);
    let _ = silently(|| signal::update::<i32>(registered, |v| *v = 1));
    assert_eq!(runs.get(), 2, "panic 的那一次也算跑过");

    // 闭包被放回、重入锁释放：已登记的依赖照常唤醒它。
    explode.set(false);
    signal::update::<i32>(registered, |v| *v = 2);
    assert_eq!(runs.get(), 3, "panic 不应让 effect 的计算闭包永久丢失");

    // 这一次跑完了，never_read 被重新登记上。
    signal::update::<i32>(never_read, |v| *v = 1);
...
fn a_signal_value_is_returned_when_the_update_closure_panics() {
    let s = signal::create(7i32);

    let _ = silently(|| {
        signal::update::<i32>(s, |v| {
            *v = 42;
            panic!("boom");
        })
    });
