...
styled! {
    pub CardHeader<'scope, Ctx><div>(#[ctx] ctx: Ctx, children: AnyView<'scope>) {
...
styled! {
    pub CardTitle<'scope, Ctx><div>(#[ctx] ctx: Ctx, children: AnyView<'scope>) {
        @apply leading-none font-semibold;
    }
}

styled! {
    pub CardDescription<'scope, Ctx><div>(#[ctx] ctx: Ctx, children: AnyView<'scope>) {
        @apply text-sm text-muted-foreground;
    }
}

styled! {
    pub CardAction<'scope, Ctx><div>(#[ctx] ctx: Ctx, children: AnyView<'scope>) {
        @apply col-start-2 row-span-2 row-start-1 self-start justify-self-end;
    }
}

styled! {
    pub CardContent<'scope, Ctx><div>(#[ctx] ctx: Ctx, children: AnyView<'scope>) {
        @apply px-6;
    }
}

styled! {
    pub CardFooter<'scope, Ctx><div>(#[ctx] ctx: Ctx, children: AnyView<'scope>) {
