...
fn main() {
    ...
    let _ = runtime.child(|scope| {
        ...
        let _keyed_view = For(context, keyed_entries, |entry| entry.id)
        ...
        let _indexed_view = Index(context, indexed_entries)
        ...
        let _stateful_view = ForStateful(context, stateful_entries, |entry| entry.id)
