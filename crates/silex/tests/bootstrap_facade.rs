...
use silex::bootstrap::{
    AppHost, BootstrapError, BrowserBootstrap, HostState, JsAppHost, LifecycleReporter,
    PageController, PageLifecyclePolicy, UnmountOutcome,
};

#[test]
fn bootstrap_api_is_exported_under_the_feature_gated_namespace() {
    let _ = std::mem::size_of::<AppHost>();
    let _ = std::mem::size_of::<BrowserBootstrap>();
    let _ = std::mem::size_of::<JsAppHost>();
