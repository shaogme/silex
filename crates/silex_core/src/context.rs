...
/// The explicit component context shared by Silex's core components.
#[derive(Clone, Copy)]
...
impl<'scope> SilexContext<'scope> {
    /// Creates a context from the scope and its root error destination.
    ...
    /// Returns the runtime scope carried by this context.
    pub fn scope(self) -> Scope<'scope> {
        self.scope
    }

    /// Returns the error destination carried by this context.
    pub fn error_reporter(self) -> ErrorReporter<'scope> {
        self.error_reporter
    }

    /// Returns this context with only its error destination replaced.
...
/// The capabilities required by a component context.
...
mod tests {
    ...
    fn replacing_reporter_preserves_scope() {
        ...
        root.with_scope(|scope| {
            ...
            let context = SilexContext::new(scope, first);

            assert!(context.with_error_reporter(second).scope() == scope);
        });
    }
}
