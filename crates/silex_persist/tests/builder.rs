...
impl<'scope> PersistenceBackend<'scope> for CallbackBackend<'scope> {
    ...
    fn subscribe(
        ...
        self.inner.subscribe(scope, key, sink)
    }
}
...
{
    Persistent::builder(scope, key.to_string())
        .backend(backend)
        .parse::<i32>()
}
...
fn manual_encode_failure_sets_write_error_for_effect_and_flush() {
    ...
    runtime.child(|scope| {
        ...
        let value = Persistent::builder(scope, "manual-encode-failure")
...
fn optional_none_flush_removes_backend_key() {
    ...
    runtime.child(|scope| {
        ...
        let value = Persistent::builder(scope, "name")
            .backend(backend.clone())
            .string()
            .optional()
            .build();
...
fn codec_callbacks_can_reenter_the_same_binding() {
    ...
    runtime.child(|scope| {
        ...
        let binding = Persistent::builder(scope, "codec-reentry")
            .backend(backend.clone())
            .custom_codec::<i32, _>(codec)
            .default(1)
            .build();
...
fn backend_callbacks_can_reenter_the_same_binding() {
    ...
    runtime.child(|scope| {
        ...
        let binding = Persistent::builder(scope, "backend-reentry")
            .backend(backend)
            .parse::<i32>()
            .default(1)
            .build();
...
fn panicking_codec_callback_restores_the_controller_payload() {
    ...
    runtime.child(|scope| {
        ...
        let binding = Persistent::builder(scope, "codec-panic")
            .backend(backend.clone())
            .custom_codec::<i32, _>(codec)
            .default(1)
            .build();
