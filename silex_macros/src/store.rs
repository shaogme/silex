...
fn build_field_initializer(field: &Field, persist_prefix: Option<&str>) -> Result<TokenStream> {
    ...
    if let Some(config) = parse_field_persist(field)? {
        ...
        Ok(quote! {
            #name: ::silex::prelude::persistent(#full_key)
                .#backend_method()
                #codec_tokens
                .default(source.#name)
                .build()
...
fn parse_field_persist(field: &Field) -> Result<Option<PersistFieldConfig>> {
    ...
    for attr in &field.attrs {
        ...
        for meta in nested {
            match meta {
                Meta::Path(path) if path.is_ident("local") => backend = Some(format_ident!("local")),
                Meta::Path(path) if path.is_ident("session") => backend = Some(format_ident!("session")),
                Meta::Path(path) if path.is_ident("query") => backend = Some(format_ident!("query")),
                Meta::NameValue(nv) if nv.path.is_ident("key") => {
                    if let syn::Expr::Lit(syn::ExprLit { lit: syn::Lit::Str(lit_str), .. }) = nv.value {
                        ...
                        return Err(syn::Error::new_spanned(nv, "persist key must be a string literal"));
                    }
                }
                Meta::NameValue(nv) if nv.path.is_ident("codec") => {
                    if let syn::Expr::Lit(syn::ExprLit { lit: syn::Lit::Str(lit_str), .. }) = nv.value {
                        ...
                        return Err(syn::Error::new_spanned(nv, "persist codec must be a string literal"));
                    }
                }
                other => {
                    return Err(syn::Error::new_spanned(other, "unsupported #[persist(...)] option"));
                }
            }
        }

        let backend = backend.ok_or_else(|| {
            syn::Error::new_spanned(attr, "#[persist(...)] requires one backend: local, session, or query")
        })?;
        let codec = codec.ok_or_else(|| {
            syn::Error::new_spanned(attr, "#[persist(...)] requires codec = \"string\" | \"parse\" | \"json\"")
        })?;

        config = Some(PersistFieldConfig { backend, codec, key });
    }
...
fn codec_builder_tokens(ty: &Type, codec: &syn::LitStr) -> Result<TokenStream> {
    match codec.value().as_str() {
        ...
        _ => Err(syn::Error::new_spanned(codec, "unsupported codec, expected string|parse|json")),
    }
}
...
mod tests {
    ...
    fn store_macro_emits_persistent_fields_for_persist_attributes() {
        ...
        assert!(expanded.contains(":: silex :: prelude :: persistent (\"settings-theme\") . local () . string ()"));
        assert!(expanded.contains(":: silex :: prelude :: persistent (\"settings-page\") . query () . parse :: < u32 > ()"));
    }
}
