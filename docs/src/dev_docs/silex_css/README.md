...
### 4.14 类名的身份：产物，不是源码文本

`css!` / `styled!` / `global!` / `tw!` 的类名是**产物的哈希**——参与哈希的是这次编译
将要注入的东西：静态 CSS、提升到全局的 CSS、每条动态规则的模板，外加决定落进哪个
`@layer` 的类名前缀。源码怎么写不参与。于是这几种写法落到同一个类名、只注入一次：
...
这里有个环：类名要写进产物（`.slx-xxx { … }`、`var(--slx-xxx-0)`），产物又要用来算
类名。解法与 §4.10 的模板同源——先拿一个控制字符占位符跑完整个生成过程，对产物取
哈希算出类名，再把占位符逐字换回真名。占位符不可能来自源码：用户字符串里的控制
字符在 `escape_css_string` 里一律被转义。

三条边界，都有断言守着：

*   **哈希的是最小化之前的中间产物**，不是最终 CSS。最小化必须先有类名，所以顺序
    上做不到；而中间产物相等一定蕴含最终 CSS 相等，这个口径只会少合、不会错合。
    代价是 `color: #ff0000` 与 `color: red` 仍是两个类名。
*   **字符串字面量逐字参与身份**。`content: "a  b"` 与 `content: "a b"` 是两段不同的
    CSS，不能因为「折叠空白后一样」就合并——合并意味着两段 CSS 抢一个类名，
    后注入的那份被 `inject_style` 按 id 丢掉，其中一处直接显示错的内容。
*   **插值表达式不参与身份**。`color: $(a)` 与 `color: $(b)` 的产物都是
    `.slx-x { color: var(--slx-x-0) }`，差别由各自元素上的行内自定义属性承担。

声明末尾的 `;` 由编译器无条件补齐，源码里写没写不进 AST（`CssDeclaration` 因此没有
`semi_token` 字段）。
...
*   **已知限制**：
    ...
    *   跨组件去重按产物（见 §4.14），但产物取的是**最小化之前**的中间形态：
        `color: #ff0000` 与 `color: red` 最小化后是同一段 CSS，却仍占两个类名。
        补上要么把类名从最终 CSS 里反推（做不到，类名先于最小化写入），要么在中间
        产物这一层再做一次值的规范化（那等于自己写一个最小化器）。
*   **性能瓶颈**：当页面存在数千个不同的动态 `Style` 对象时，虽然 DOM 压力小，但 Rust 端的 `Effect` 闭包管理会有一定的内存开销。
*   **TODO**：
    1.  [ ] 打通 SSR：给 `runtime/backend.rs` 加一个累积 CSS 文本的服务端后端，并等
