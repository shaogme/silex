...
*   **强类型校验与 `CssVar<T>`**：
    *   **原理**：`define_theme!` 生成的常量（如 `AppTheme::PRIMARY`）并非普通字符串，而是 `CssVar<Hex>` 类型。
...
*   **自动化宏**：`define_theme!` 宏高度集成了 `Patch` 结构体生成、常量生成和转换逻辑，开发者只需定义结构，即可获得完整的工具链。
