...
Silex 提供了一个能够与 CSS 变量无缝集成的响应式主题系统。

*   **零 DOM 损耗**：`theme_variables` 并没有创建阴影 DOM 或包裹节点，而是利用 `apply` 机制将主题变量映射直接绑定到现有元素的 `style` 对象上。
*   **局部补丁 (Partial Patching)**：
    *   **实现原理**：利用 CSS 变量的继承链。当一个元素被应用了 `theme_patch` 时，它仅会 `set_property` 补丁中指定的变量。
    *   **回退机制**：如果补丁中的某个字段为 `None`，则会触发 `remove_property`。浏览器会自动从父级作用域或 `:root` 继承该变量的值。
*   **IntoSignal 重构**：
    *   为了提升 API 的灵活性，所有主题 API 现已改为接收 `impl IntoSignal`。
    *   这允许开发者直接将 `rx!` 宏生成的匿名计算节点传入 `theme_patch`，而无需手动包装为 `Signal`。
*   **宏自动化**：`define_theme!` 宏现在通过派生机制自动生成对应的 `Patch` 结构体及其链式 Setter，这大大减少了手动维护局部覆盖逻辑的工作量。
