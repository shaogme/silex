...
| 模块/文件 | 职责说明 |
| :--- | :--- |
| `src/builder.rs` | 提供 `Style` 结构体及链式调用 API，支持静态/动态样式提取。 |
...
| `src/types/` | 模块化的类型系统，包含单位 (`units.rs`)、计算 (`calc.rs`)、复合属性 (`shorthands.rs`) 及渐变 (`gradients.rs`)。 |
| `src/types.rs` | 类型系统入口，定义核心验证 Trait `ValidFor` 并整合属性注册。 |
...
1.  **哈希生成**: 使用 `CssHasher` 对 *属性名* + *静态值* + *伪类* 组合计算唯一指纹。**动态值不参与哈希**，确保持续更新时不产生新样式类。
2.  **变量注入**: 动态信号被映射为 CSS 变量（如 `--sb-{hash}-{index}`），写入样式表的类定义中。
3.  **原子更新**: 渲染时产生极轻量 `Effect`，直接调用 `CSSStyleDeclaration.setProperty` 更新变量值。
...
| 实体 | 路径 | 说明 |
| :--- | :--- | :--- |
| `Style` | `builder.rs` | 链式构建器。方法包括 `width()`, `height()`, `on_hover()` 等。 |
| `Px(f64)`, `Rem(f64)` | `types.rs` | 标准 CSS 单位包装。 |
| `UnsafeCss(String)` | `types.rs` | 逃逸舱，用于绕过类型检查，通过 `UnsafeCss::new("calc(...)")` 调用。 |
...
| `theme_variables(theme)` | `theme.rs` | 零开销主题变量注入，不引入代理节点。 |
| `set_global_theme(theme)` | `theme.rs` | 全局 `:root` 主题挂载。 |
