...
const EXTENSIONS: &[&str] = &["groovy"];

impl LanguageAnalyzer for GroovyAnalyzer {
    fn descriptor(&self) -> LanguageDescriptor {
        LanguageDescriptor::new(LanguageId::Groovy, EXTENSIONS, &[])
    }
...
fn is_test_declaration(line: &str) -> bool {
    let trimmed = line.trim_start();
    trimmed.starts_with("def test") || trimmed.starts_with("@Test")
}
