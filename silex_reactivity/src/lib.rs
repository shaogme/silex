    ...
    RUNTIME.with(|rt| {
        {
            if let Some(signal) = rt.signals.get_mut(id) {
                ...
                return; // Dropped or not a signal
            }
        }
    ...
    F: Fn(Option<&T>) -> T + 'static,
{
    RUNTIME.with(|rt| {
        ...
        // State Component
        rt.states.insert(id, NodeState::Clean);
        ...
        let computation = move || {
            ...
            if changed {
                RUNTIME.with(|rt| {
                    if let Some(signal) = rt.signals.get_mut(id) {
                        signal.value = AnyValue::new(new_value);
                        signal.version = signal.version.wrapping_add(1);
                    }
                    rt.queue_dependents(id);
                });
        ...
        if let Some(effect) = rt.effects.get_mut(id) {
...
pub fn register_closure(f: Box<dyn Any>) -> NodeId {
    RUNTIME.with(|rt| rt.register_closure_internal(f))
}

pub fn try_with_closure<T: 'static, R>(id: NodeId, f: impl FnOnce(&T) -> R) -> Option<R> {
    RUNTIME.with(|rt| {
        if let Some(data) = rt.closures.get(id)
