...
fn App<'scope>(
    ...
    let theme = store.theme.map(scope, |name| css::get_theme(name.as_str()));

    let routes = routes!(AppRoutes {
        home "/" => move |ctx| routes::HomePage(ctx).build(),
        basics "/basics" => move |_ctx| basics::BasicsPage(scope).build(),
        flow "/flow" => move |_ctx| flow_control::FlowPage(scope).build(),
        ...
        nest css "/css" => move |ctx, outlet| routes::CssLayout(ctx, outlet).build() {
            basics "/" => move |_ctx| css::StylingBasics(scope).build(),
            theming "/theming" => move |_ctx| css::Theming(scope, store).build(),
            advanced "/advanced" => move |_ctx| css::AdvancedStyling(scope).build(),
        },
        nest advanced "/advanced" => move |ctx, outlet| {
            routes::AdvancedLayout(ctx, outlet).build()
        } {
            index "/" => move |_ctx| routes::SelectDemo().build(),
            store "/store" => move |_ctx| advanced::StoreDemo(scope, store).build(),
